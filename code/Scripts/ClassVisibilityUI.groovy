    ...
    c.selectedProperty().addListener({o, oldV, newV ->
        ...
        for (n in firstCol){
            if (n.getId() == "None"){
                ...
                className = getPathClass(n.getId())
            }
            if (n.isSelected()){
                getCurrentViewer().getOverlayOptions().hiddenClassesProperty().remove(className)
            } else {getCurrentViewer().getOverlayOptions().hiddenClassesProperty().add(className)}
        }
    } as ChangeListener)
}
